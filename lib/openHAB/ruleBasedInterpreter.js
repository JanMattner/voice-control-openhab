...
class CommandExp extends Expression {
    ...
    evaluate(tokens) {
        ...
        var sendCommand = function(parameter) {
        ...
        return new EvaluationResult(true, result.remainingTokens, sendCommand, result.executeParameter);
    }
}
    ...
    interpretUtterance(utterance) {
        ...
        for (var index = 0; index < this.rules.length; index++) {
            ...
            if (result.success) {
                ...
                result.success = executeFunction(result.executeParameter);
                break;
            }
        }
 ...
 * @returns {AnnotationResult}
 */
function annotateResult(result, inputTokens) {
...
if (process.env.NODE_ENV === "test") {
   module.exports.annotateResult = annotateResult;
