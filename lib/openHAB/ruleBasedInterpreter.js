...
function itemLabel() {return new ItemLabelExp();}
class ItemLabelExp extends Expression {
    ...
    evaluate(tokens) {
        ...
        let matchResult = getItemByLabelOrSynonym(allItems, tokens);
        ...
        return new EvaluationResult(true, matchResult.remainingTokens, null, {items: [matchResult.matchedItem]});
    }
}
 ...
 */
function itemProperties(expression, tags, locationRequired) {return new ItemPropertiesExp(expression, tags, locationRequired);}
class ItemPropertiesExp extends Expression {
     ...
     * @param {boolean} locationRequired 
     */
    constructor(expression, tags = [], locationRequired = true) {
        super();
        this.value = expression;
        this.tags = tags;
    ...
    evaluate(tokens) {
        ...
        let remainingItems = items.getItemsByTag(...this.tags);
...
function locationLabel() {return new LocationLabelExp();}
class LocationLabelExp extends Expression {
    constructor() {
        super();
    }
    ...
    evaluate(tokens) {
        ...
        let locationItems = getItemsBySemanticType(items.getItems(), "Location").filter(item => item.type == "GroupItem");
        
        let matchResult = getItemByLabelOrSynonym(locationItems, tokens);
        ...
        return new EvaluationResult(true, matchResult.remainingTokens, null, null, matchResult.matchedItem);
    }
}
